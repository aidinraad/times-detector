...
This repo contains an application of [YOLOv2 Keras/Tensorflow](https://github.com/experiencor/keras-yolo2.git) for times magazine detection.
...
The object annotation are done by [yolo_mark](https://github.com/AlexeyAB/Yolo_mark.git), generating annotation in VOC format. The source code for generating training images cant be found at ```./gen-times-images/igenerate_trainimgs.py```.
Download generated images from [here](https://1drv.ms/u/s!Avf8jJ1bO4r4gWOMqfn0JXzvKgnp). The `data` folder containing times train and valid images shall be placed at project root directory.
```
...
```./conf.json``` is already set up for the current implementation.
...
```
Trained weights will be save at ```./full_yolo_times.h5```
...
## 3. Perform detection:
The current detection results can be downloaded from [here](https://1drv.ms/u/s!Avf8jJ1bO4r4gWQ-JOyS9jy-XhkI).
Perform detection on valid set. Results are stored in ```./results/valid```
```
python predict_.py -c ./config.json -w ./full_yolo_times.h5 -i ./data/times/valid/images/ -o ./results/valid/
```
Perform detection train set. Results are stored in ```./results/train```
```
python predict_.py -c ./config.json -w ./full_yolo_times.h5 -i ./data/times/train/images/ -o ./results/train/
```
# Perform detection on video. Results are stored in ```./video```
```
